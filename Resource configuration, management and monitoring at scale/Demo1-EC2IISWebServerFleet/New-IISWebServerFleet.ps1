...
(
    ...
    # The size of instances to be launched in the fleet. If not specified burstable t3.large will be assumed.
...
function _addNameTagToResource([string]$ResourceID, [string]$NameTagValue)
{
    ...
    do {
        try {
            ...
            # error possibly due to eventual consistency, so wait and try once more
            if ($attempt -lt $maxAttempt) {
                $attempt++
                Write-Host "... waiting for resource to become available for tagging"
