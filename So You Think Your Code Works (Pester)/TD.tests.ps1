...
Describe "MockExample" {
    ...
    Context "Strip Vowels with a real file" {
        add-content -path "TestDrive:/MyTestFile.txt" -value "Powershell On The River!"
        
        It "Doesn't Throw when the file exists and has content" {
            {Strip-Vowels -filePath "TestDrive:/MyTestFile.txt"} | should -Not -Throw
        }
        
        It "Doesn't return an empty string" {
            $Result = Strip-Vowels -FilePath "TestDrive:/MyTestFile.txt"
            $Result | Should -Not -BeNullOrEmpty
        }

        It "Returns the correct value for the string" {
            $Result = Strip-Vowels -FilePath "TestDrive:/MyTestFile.txt"
            $Result | Should -BeExactly "Pwrshll n Th Rvr!"
        }

        It "really created a file" {
            test-path "TestDrive:/MyTestFile.txt" | should -be $true
        }
    }
